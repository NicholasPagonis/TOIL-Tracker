...
exports.calculateSessionMinutes = void 0;
const express_1 = require("express");
const prisma_1 = require("../lib/prisma");
const til_1 = require("../lib/til");
Object.defineProperty(exports, "calculateSessionMinutes", { enumerable: true, get: function () { return til_1.calculateSessionMinutes; } });
const schemas_1 = require("../validation/schemas");
const error_1 = require("../middleware/error");
const til_2 = require("../lib/til");
const router = (0, express_1.Router)();
// POST /api/sessions/clock-in
router.post("/clock-in", async (req, res, next) => {
    try {
        const body = schemas_1.ClockInSchema.parse(req.body);
        const now = new Date();
        const startedAt = body.startedAt ? new Date(body.startedAt) : now;
        // Check for open session within last 8 hours
        const eightHoursAgo = new Date(now.getTime() - 8 * 60 * 60 * 1000);
        const openSession = await prisma_1.prisma.session.findFirst({
            where: {
                endedAt: null,
        ...
        const source = req.headers["x-api-key"] && process.env.API_KEY
            ? "SHORTCUT"
            : "MANUAL";
...
router.post("/clock-out", async (req, res, next) => {
    try {
        const body = schemas_1.ClockOutSchema.parse(req.body);
        const now = new Date();
        ...
        if (!openSession) {
            return next((0, error_1.createError)("No open session found. Please clock in before clocking out.", 409));
        }
...
router.get("/", async (req, res, next) => {
    try {
        ...
        if (query.from || query.to) {
            where.startedAt = {};
            if (query.from) {
                where.startedAt.gte = (0, til_2.perthDateToUtcStart)(query.from);
            }
            if (query.to) {
                where.startedAt.lte = (0, til_2.perthDateToUtcEnd)(query.to);
            }
        }
...
router.post("/", async (req, res, next) => {
    try {
        const body = schemas_1.CreateSessionSchema.parse(req.body);
        ...
        const overlapping = await prisma_1.prisma.session.findFirst({
        ...
        res.status(201).json({
            ...
            ...(overlapping
                ? {
                    warning: "This session overlaps with an existing session. Please review.",
...
router.patch("/:id", async (req, res, next) => {
    try {
        ...
        const body = schemas_1.UpdateSessionSchema.parse(req.body);
        const existing = await prisma_1.prisma.session.findUnique({ where: { id } });
        if (!existing) {
            return next((0, error_1.createError)("Session not found", 404));
        }
        const updateData = { source: "EDITED" };
        ...
        if (body.breaks !== undefined) {
            await prisma_1.prisma.break.deleteMany({ where: { sessionId: id } });
            await prisma_1.prisma.break.createMany({
